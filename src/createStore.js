...
export default function createStore(reducer, preloadedState, enhancer) {
  ...
  function dispatch(action) {
    ...
    try {
      isDispatching = true
      currentState = currentReducer(currentState, action)  // 执行reducer 函数，获取到当前的 currentState
    } finally {
      isDispatching = false
    }

    const listeners = (currentListeners = nextListeners)  // 执行订阅者
  ...
  function observable() {
    ...
    return {
      ...
      subscribe(observer) {
        ...
        observeState()  // 对 State 进行观察
        const unsubscribe = outerSubscribe(observeState)  // 对 state 订阅
        return { unsubscribe }
      },
